// tslint:disable:max-line-length
export const ru = {
  shared: {
    ok: 'Ok',
    save: 'Сохранить',
    search: 'Поиск',
    settings: 'Настройки',
    searchSettings: 'Настройки поиска',
    totalResults: 'Количество результатов: ',
    fieldIsRequiredError: 'Поле обязательно',
    fieldMinLengthError: 'Поле должно быть не меньше {{minCharacters}} символов',
    fieldMaxLengthError: 'Поле должно быть не более {{maxCharacters}} символов',
    header: {
      searchFor: 'Искать:',
      users: 'Пользователей',
      repositories: 'Репозитории',
      signUp: 'Зарегистрироваться',
      login: 'Войти',
      logOut: 'Выйти',
      user: 'Пользователь',
    },
    footer: {
      fsd: 'Fullstack Development',
    },
    notifications: {
      saved: 'Данные пользователя сохранены.',
    },
  },
  features: {
    profile: {
      name: 'Имя',
      nickname: 'Псевдоним',
      age: 'Возраст',
      bio: 'Биография',
      editProfile: 'Редактировать профиль',
      yearsOld: '{{count}} лет',
      yearsOld_1: '{{count}} года',
      yearsOld_2: '{{count}} год',
      edit: 'Редактировать',
    },
    repositoriesSearch: {
      minStars: 'Мин. звезд',
      minForks: 'Мин. форков',
      repositoriesLanguage: 'Язык репозиториев',
      owner: 'Владелец',
      language: 'Язык',
      forks: 'Форки',
      openIssues: 'Открытые задачи',
      lastUpdated: 'Последнее обновление',
    },
    userSearch: {
      usersSearch: 'Поиск пользователей GitHub',
      repositoriesSearch: 'Поиск репозиториев GitHub',
      user: 'Пользователей',
      users: 'Пользователей',
      organizations: 'Организации',
      usersAndOrganizations: 'Пользователей и организации',
      usernameAndEmail: 'Никнейму и почте',
      username: 'Никнейму',
      email: 'Почте',
      fullName: 'Полному имени',
      both: 'Пользователей и организации',
      followers: 'Подписчики',
      following: 'Подписки',
      repositories: 'Репозитории',
      userDetails: 'Данные пользователя',
      min: 'Мин.',
      max: 'Макс.',
      minRepos: 'Мин. репозиториев',
      maxRepos: 'Макс. репозиториев',
      searchBy: 'Поиск по',
      searchFor: 'Искать',
      repositoriesLanguage: 'Язык репозиториев',
      resultsPerPage: 'Результ. на страницу',
      repositoriesNumber: 'Количество репозиториев',
    },
    authorization: {
      buttonSignUp: 'Зарегистрироваться',
      buttonLogin: 'Войти',
      buttonRestore: 'Отправить новый пароль',
      or: 'или',
      email: 'Email',
      password: 'Пароль',
      hide: 'Скрыть',
      show: 'Показать',
      signUp: 'Регистрация',
      emailWarning: 'Email должен содержать «@»',
      ruleLowcaseLetter: 'Одна строчная буква',
      ruleUppercaseLetter: 'Одна заглавная буква',
      ruleDigit: 'Одна цифра',
      ruleMinSigns: 'Минимум 8 знаков',
      login: 'Войти',
      buttonLogout: 'Выйти',
      restore: 'Восстановить пароль',
      restorePassword: 'Восcтановить пароль',
      restoreText: 'Напомните нам вашу почту, и мы пришлем новый пароль',
      termsOfUseText: 'Нажимая на кнопку «Зарегистрироваться», вы подтверждаете свое согласие с условиями предоставления услуги',
      termsOfUse: 'Пользовательское соглашение',
      mailing:'Я не хочу получать еженедельную рассылку с советами по поиску работы и новостях о самых востребованных профессиях',
      user: 'Пользователь'
    },
  },
  documents: {},
  modules: {},
};
